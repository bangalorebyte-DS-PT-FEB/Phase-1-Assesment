...
class UserDBManager:
	...
	def create_client(self, username, password):
		...
		c.execute("""INSERT INTO users(username, password, permission_level) VALUES(?, ?, ?)""",(username, password, 'client'))
		conn.commit()
	...
	def create_account (self, username):
		...
		c.execute("""INSERT INTO accounts(cash, portfolio_worth, user_id) VALUES(?, ?, ?)""",(100000, 0, client_id))
		conn.commit()

	def create_admin(self, username, password):
		c.execute("""INSERT INTO users(username, password, permission_level) VALUES(?, ?, ?)""",(username, password, 'admin'))
		conn.commit()
...
class TransactionsDBManager:
	def buy(self, username, num_shares, ticker, last_price):
		...
		c.execute("""INSERT INTO transactions(type, account_id, ticker, num_shares)	VALUES(?, ?, ?, ?)""",("buy", account_id, ticker, num_shares))
		conn.commit()
	...
	def sell(self, username, num_shares, ticker, last_price):
		...
		#transaction
		c.execute("""INSERT INTO transactions(type, account_id, ticker, num_shares) VALUES(?, ?, ?, ?)""",("sell", account_id, ticker, num_shares))
		conn.commit()
